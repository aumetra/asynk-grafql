...
use crate::{ParseRequestError, futures_util::TryStreamExt, http::MultipartOptions};
use axum::{
...
impl<R> GraphQLRequest<R> {
    /// Unwraps the value to `crate::Request`.
    #[must_use]
...
pub struct GraphQLBatchRequest<R = rejection::GraphQLRejection>(
    pub crate::BatchRequest,
    PhantomData<R>,
);

impl<R> GraphQLBatchRequest<R> {
    /// Unwraps the value to `crate::BatchRequest`.
    #[must_use]
    pub fn into_inner(self) -> crate::BatchRequest {
        self.0
    }
}
    ...
    R: IntoResponse + From<ParseRequestError>,
{
    ...
    async fn from_request(req: Request, _state: &S) -> Result<Self, Self::Rejection> {
        if req.method() == Method::GET {
            let uri = req.uri();
            let res = crate::http::parse_query_string(uri.query().unwrap_or_default())
                .map_err(|err| {
            ...
            Ok(Self(crate::BatchRequest::Single(res?), PhantomData))
        } else {
