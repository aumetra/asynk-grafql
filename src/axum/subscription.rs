...
where
    ...
    OnConnInitFut: Future<Output = crate::Result<Data>> + Send + 'static,
    OnPing: FnOnce(Option<&Data>, Option<serde_json::Value>) -> OnPingFut + Clone + Send + 'static,
    OnPingFut: Future<Output = crate::Result<Option<serde_json::Value>>> + Send + 'static,
{
    ...
    where
        ...
        R: Future<Output = crate::Result<Data>> + Send + 'static,
    {
        GraphQLWebSocket {
            sink: self.sink,
            stream: self.stream,
    ...
    pub async fn serve(self) {
        ...
        let stream =
            crate::http::WebSocket::new(self.executor.clone(), input, self.protocol.0)
